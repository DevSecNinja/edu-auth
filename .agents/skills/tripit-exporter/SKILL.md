...
Export all TripIt travel data (trips, flights, lodging, activities) to JSON
format using TripIt's undocumented API v2.
...
- **TripIt account**: The user must be able to log in to tripit.com.
...
> The `X-Requested-With: XMLHttpRequest` header is required. Without it, the v2
> endpoints return `401 Unauthorized` even when a valid session cookie is
> present.
...
**Tell the user to log in manually, then confirm when done.**
...
- **401 Unauthorized**: Either the session expired, or the
  `X-Requested-With: XMLHttpRequest` header is missing. Confirm the header is
  set on every request; if it is, re-authenticate by navigating to the TripIt
  login page.
...
- The `X-Requested-With: XMLHttpRequest` header is required on every request;
  without it the v2 endpoints return `401` even with a valid session cookie.

## Attribution

Adapted from Casey Grippi's `tripit-export` skill. Redistributed for personal
use. Skills contain hardcoded assumptions (locations, accounts, preferences)
that you may need to adapt for your own use. See
[`LICENSE`](LICENSE) and the repository
[`THIRD_PARTY_NOTICES.md`](../../../apm_modules/DevSecNinja/ai-toolkit/THIRD_PARTY_NOTICES.md).

- **Source:** <https://lobehub.com/skills/caseyg-caseys-claude-tripit-export>
